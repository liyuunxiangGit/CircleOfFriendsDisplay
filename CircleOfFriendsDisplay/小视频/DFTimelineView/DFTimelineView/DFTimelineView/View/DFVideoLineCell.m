...
#define      HighLightTextColor  [UIColor redColor]
    ...
    if (_videoItem.videoUrl != nil && ![_videoItem.videoUrl isEqualToString:@""]) {
        ...
        if ([manager fileExistsAtPath:filePath]) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
                NSData  *data = [NSData dataWithContentsOfURL:[NSURL URLWithString:self->_videoItem.videoUrl]];
...
-(void) decodeVideo
{
    if (_videoItem.decorder == nil) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            DFVideoDecoder *decoder = [[DFVideoDecoder alloc] initWithFile:self->_videoItem.localVideoPath];
            decoder.delegate = self;
            self->_videoItem.decorder = decoder;
            [decoder decode];
        });
    }else{
...
-(void)onDecodeFinished
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self->_videoItem.decorder.animation != nil) {
            [self->_videoView.layer removeAnimationForKey:@"contents"];
            [self->_videoView.layer addAnimation:self->_videoItem.decorder.animation forKey:nil];
