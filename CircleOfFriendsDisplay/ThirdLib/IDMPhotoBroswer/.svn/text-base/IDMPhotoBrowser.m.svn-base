...
// Private Properties
@property (nonatomic, strong) LCActionSheet *actionsSheet;
...
// Properties
@synthesize displayDoneButton = _displayDoneButton, displayUseButton = _displayUseButton, displayToolbar = _displayToolbar, displayActionButton = _displayActionButton, displayCounterLabel = _displayCounterLabel, useWhiteBackgroundColor = _useWhiteBackgroundColor, doneButtonImage = _doneButtonImage, useButtonImage = _useButtonImage;
...
- (void)viewDidLoad {
    ...
    [_useButton setFrame:[self frameForUseButtonAtOrientation:currentOrientation]];
    [_useButton setAlpha:1.0f];
    [_useButton addTarget:self action:@selector(useButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if(!_doneButtonImage) {
        ...
        _doneButton.frame = CGRectMake(_doneButton.frame.origin.x, _doneButton.frame.origin.y, _doneButtonImage.size.width, _doneButtonImage.size.height);
    ...
    if(!_useButtonImage) {
        [_useButton setTitleColor:[UIColor colorWithWhite:0.9 alpha:0.9] forState:UIControlStateNormal|UIControlStateHighlighted];
        if (_useButtonTitle != nil && [_useButtonTitle isEqual:[NSNull null]] == NO && _useButtonTitle.length > 0) {
            [_useButton setTitle:_useButtonTitle forState:UIControlStateNormal];
        }else{
            [_useButton setTitle:IDMPhotoBrowserLocalizedStrings(@"Use") forState:UIControlStateNormal];
        }
        [_useButton.titleLabel setFont:[UIFont boldSystemFontOfSize:11.0f]];
        [_useButton setBackgroundColor:[UIColor colorWithWhite:0.1 alpha:0.5]];
        _useButton.layer.cornerRadius = 3.0f;
        _useButton.layer.borderColor = [UIColor colorWithWhite:0.9 alpha:0.9].CGColor;
        _useButton.layer.borderWidth = 1.0f;
    }
    else {
        [_useButton setBackgroundImage:_useButtonImage forState:UIControlStateNormal];
        _useButton.contentMode = UIViewContentModeScaleAspectFit;
    }
    ...
    // Action Button
    _actionButton = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"更多"] style:UIBarButtonItemStyleBordered target:self action:@selector(actionButtonPressed:)];
...
- (void)viewWillLayoutSubviews {
    ...
    _useButton.frame = [self frameForUseButtonAtOrientation:currentOrientation];
...
- (void)performLayout {
    ...
    if(_displayUseButton && !self.navigationController.navigationBar)
...
- (CGRect)frameForDoneButtonAtOrientation:(UIInterfaceOrientation)orientation {
    ...
    if (_doneButtonImage != nil) {
        return CGRectMake(15, 30, _doneButtonImage.size.width, _doneButtonImage.size.height);
    }else{
        return CGRectMake(15, 30, 55, 26);
    }
}

- (CGRect)frameForUseButtonAtOrientation:(UIInterfaceOrientation)orientation {
    CGRect screenBound = self.view.bounds;
    CGFloat screenWidth = screenBound.size.width;
    
    // if ([self isLandscape:orientation]) screenWidth = screenBound.size.height;
    
    return CGRectMake(screenWidth - 55, 30, 55, 26);
}
...
- (void)useButtonPressed:(id)sender {
    ...
    if ([_delegate respondsToSelector:@selector(photoBrowser:didPressUseButtonAtIndex:)]) {
        [_delegate photoBrowser:self didPressUseButtonAtIndex:_currentPageIndex];
    }
}
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        {
            // Action sheet
            //            self.actionsSheet = [LCActionSheet new];
            //            self.actionsSheet.delegate = self;
            //            for(NSString *action in _actionButtonTitles) {
            //                [self.actionsSheet addButtonWithTitle:action];
            //            }
            self.actionsSheet = [[LCActionSheet alloc]initWithTitle:nil buttonTitles:_actionButtonTitles redButtonIndex:-1 delegate:self];
